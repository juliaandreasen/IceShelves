...
A=dlmread('pinei_area1.prn'); %read in area change datasets from ice shelf
x=A(:,1); %define x column 
y=A(:,2); %define y column 
Z=A(1:10,1); %disregard last date because there is no difference data
...
cc2= [.969 .545 .055; .969 .878 .055; .275 .824 .035; .035 .824 .733; .035 .533 1; .09 .035 .824; .471 .133 .91; .886 .133 .91; .976 .388 .714; 1 .89 .953];
colormap(cc2);
...
ylabel('Ice Shelf Area Difference (km^2)', 'FontSize',14); %y-axis label and fontsize

xlim([2008 2019]); %set the year range on the x-axis 
ylim([-650 250]); %set the area difference range on the y-axis
title('Yearly Pine Island Ice Shelf Area Difference 2009-2019'); %create a graph title

% module C: save resulting figure
filename = 'pinei_areadiffbar.png' %define the filename
saveas(fHand,'pinei_areadiffbar.png') %save the bar graph figure under this name
