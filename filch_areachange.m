...
A=dlmread('filch_area1.prn'); %read in area change datasets from ice shelf 
...
set (gca, 'fontsize', 14,'linewidth', 1) %set the current bar plot axis to a fontsize of 14 and lindwidth of 1 
xlabel('Year','FontSize',14); %x-axis label and fontsize
ylabel('Ice Shelf Area (km^2)', 'FontSize',14); %y-axis label and fontsize
title('Filchner Ice Shelf Area Change 2009-2019'); %create a graph title
...
for i=1:length(x)
    ...
    ylim([103000 107000]); %set the area range on the y-axis
...
filename = 'filch_areachange.gif'; %specify the output file name
